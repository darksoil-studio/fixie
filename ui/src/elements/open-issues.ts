 ...
 */
@localized()
@customElement("open-issues")
export class OpenIssues extends SignalWatcher(LitElement) {
  /**
   * @internal
   */
    ...
    if (hashes.length === 0) {
      return html` <div class="column center-content" style="gap: 16px; flex: 1">
        <sl-icon
          ...
          ></sl-icon
        >
        <span class="placeholder">${msg("No issues found.")}</span>
      </div>`;
    }

    return html`
      <div class="column" style="gap: 16px; flex: 1">
        ${hashes.map(hash => html`<issue-summary .issueHash=${hash}></issue-summary>`)}
      </div>
    `;
  }

  render() {
    const map = this.fixieStore.openIssues.get();
    ...
    switch (map.status) {
      ...
      case "error":
        return html`<display-error
          .headline=${msg("Error fetching the issues")}
          .error=${map.error}
        ></display-error>`;
      case "completed":

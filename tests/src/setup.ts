...
import { FixieClient } from '../../ui/src/fixie-client.js';
import { FixieStore } from '../../ui/src/fixie-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/fixie_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new FixieStore(
  	new FixieClient(player.appWs as any, 'fixie_test'),
  );
  return {
  	store,
  	player,
